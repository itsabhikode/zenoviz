...
async def test_register_success_returns_message() -> None:
    svc, fake = _service()
    response = await svc.register(RegisterRequest(email="u@example.com", password="Pass1!"))
    assert "successful" in response.message.lower()
    assert fake.registered == [("u@example.com", "Pass1!")]
...
async def test_register_propagates_value_error() -> None:
    ...
    with pytest.raises(ValueError, match="already registered"):
        await svc.register(RegisterRequest(email="u@example.com", password="Pass1!"))
