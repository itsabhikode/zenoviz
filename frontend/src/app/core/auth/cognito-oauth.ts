/** Session keys for Cognito Hosted UI (PKCE) redirect flow. */
...
export function cognitoAuthorizeUrl(hostedUiDomain: string): string {
  const host = hostedUiDomain.replace(/^https?:\/\//, '').replace(/\/$/, '');
  return `https://${host}/oauth2/authorize`;
}
...
export function buildGoogleAuthorizeUrl(params: {
  ...
  const base = cognitoAuthorizeUrl(params.hostedUiDomain);
